        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <h4 class="col-auto mr-auto">Data Pengurus</h4>
                        @if (Auth::user()->getRoleNames()->first() == 'admin')
                            <a href="{{ route('pengurus.create') }}" class="btn btn-primary btn-sm col-auto">Tambah
                                Pengurus</a>
                        @endif
                    </div>
                </div>
...
@push('js')
    ...
    {{ $dataTable->scripts() }}
    <script>
        $('#pengurus-table').on('click', '.action', function() {
            let data = $(this).data();
            let id = data.id;
            let jenis = data.type;
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                $('#pengurus-table').DataTable().ajax.reload();
                            }
                        });
                    }
                })
